<?php 
   ...
   if (!empty($_POST))
   {
       ...
       if(empty($name))
       {
           $nameError = 'Ce champs ne doit pas être vide';
           $isSuccess = false;
       }
       if(empty($description))
       {
           $descriptionError = 'Ce champs ne doit pas être vide';
           $isSuccess = false;
       }
       if(empty($price))
       {
           $priceError = 'Ce champs ne doit pas être vide';
           $isSuccess = false;
       }
       if(empty($category))
       {
           $categoryError = 'Ce champs ne doit pas être vide';
           $isSuccess = false;
       }
       if(empty($image))
       {
           $imageError = 'Ce champs ne doit pas être vide';
           $isSuccess = false;
       }
       else
       {
       ...
       {
           $db = Cnx::connect();
           $statement = $db->prepare("INSERT INTO articles (designation, description, prixUnitaire, categorie, imageArticle) values(?, ?, ?, ?, ?)");
...
<head>
    ...
    <h1><strong>Ajouter un article</strong></h1> 
        ...
        <div class="form-group">
        <label for="imageArticle">Sélectionner une image:</label>
